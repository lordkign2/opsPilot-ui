...
const mockOrders: OrderItem[] = [
  {
    id: '#OP-9921',
    customerName: 'Amina Tech Logistics',
    customerEmail: 'procurement@aminatech.ng',
    itemsCount: '12 items',
    amount: '₦450,000.00',
    status: 'PROCESSING',
    created: 'Oct 24, 14:30',
  },
  {
    id: '#OP-9920',
    customerName: 'Global Trade Co.',
    customerEmail: 'orders@globaltrade.co',
    itemsCount: '3 items',
    amount: '₦125,000.00',
    status: 'PENDING',
    created: 'Oct 24, 11:15',
  },
  {
    id: '#OP-9919',
    customerName: 'Nexus Industries',
    customerEmail: 'contact@nexus.ng',
    itemsCount: '24 items',
    amount: '₦1,200,000.00',
    status: 'COMPLETED',
    created: 'Oct 23, 16:45',
  },
  {
    id: '#OP-9918',
    customerName: 'Delta Suppliers',
    customerEmail: 'sales@deltasup.com',
    itemsCount: '1 item',
    amount: '₦45,000.00',
    status: 'CANCELLED',
    created: 'Oct 23, 09:20',
  },
];
      ...
      {viewMode === 'table' ? (
        ...
        <div className="glass-card rounded-custom-md border border-white/5 bg-slate-900/10 overflow-hidden shadow-xl">
                ...
                {filteredOrders.map((ord) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 font-mono font-bold text-primary">
                      <Link href={`/orders/${ord.id.slice(1)}`} className="hover:underline flex items-center space-x-1.5">
                        <span>{ord.id}</span>
                    ...
                    <td className="px-6 py-4 text-center">
                      <Link href={`/orders/${ord.id.slice(1)}`}>
          ...
          <div className="bg-[#0b1120]/40 px-6 py-4 border-t border-white/5 flex items-center justify-between text-xs text-text-secondary">
            <span>
              ...
              <span className="text-white font-medium">1,204</span> entries
            </span>
            <div className="flex items-center space-x-4">
              <span className="text-text-muted">Page 1 of 301</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {(['PENDING', 'PROCESSING', 'COMPLETED', 'CANCELLED'] as const).map((col) => {
            const colOrders = mockOrders.filter((ord) => ord.status === col);
            return (
              <div key={col} className="glass-card rounded-custom-md border border-white/5 bg-slate-900/10 flex flex-col h-[520px]">
                ...
                <div className="flex-1 p-3 overflow-y-auto space-y-3">
                  {colOrders.map((ord) => (
                    <Link href={`/orders/${ord.id.slice(1)}`} key={ord.id} className="block">
                      <div className="p-4 bg-slate-900 border border-white/5 rounded-custom-sm hover:border-primary/50 hover:shadow-[0_0_10px_rgba(0,245,255,0.05)] transition-all space-y-3 cursor-pointer relative group">
                        <div className="flex justify-between items-start">
                          <span className="text-xs font-bold text-primary font-mono">{ord.id}</span>
